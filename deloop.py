...
class Deloop_OT_dering(Operator):
    bl_idname = 'deloop.dering'
    bl_label = 'Dering'
    bl_description = 'Dering'
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        Deloop.dering_by_selection(
            context=context
        )
        return {'FINISHED'}
...
def register(ui=True):
    ...
    register_class(Deloop_OT_dering)
    if ui:
...
def unregister(ui=True):
    ...
    unregister_class(Deloop_OT_dering)
